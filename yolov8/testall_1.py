...
alert_sound = "D:/yolov8/mp3/luvvoice.com-20250318-1qRGUf.mp3"
    ...
    if cap.isOpened():
        ...
        if ret:
            ...
            if current_count >= p38_per_layer:
                ...
                play_sound()  # 🔊 เล่นเสียงแจ้งเตือนเมื่อครบ 34 ชิ้น  
...
# ✅ ตัวแปรควบคุมโหมด Brake
is_brake_mode = False  # ตรวจสอบว่าอยู่ในโหมด Brake หรือไม่
brake_image = None  # เก็บภาพนิ่งเมื่อกดปุ่ม Brake

# ✅ ฟังก์ชัน Toggle Brake Mode (ทำให้หยุดกล้องและแสดงข้อความทันที)
def toggle_brake():
    global is_brake_mode, brake_image, cap

    if not is_brake_mode:
        # **เข้าสู่โหมด Brake**
        is_brake_mode = True
        update_status("Brake")  # ✅ อัปเดตสถานะ

        # **หยุดกล้องทันที**
        if cap and cap.isOpened():
            ret, frame = cap.read()
            cap.release()  # ปิดกล้อง
            camera_label.after_cancel(update_frame)  # หยุดอัปเดตเฟรม
            
            if ret:
                # **บันทึกภาพนิ่ง**
                frame = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)
                img = Image.fromarray(frame)
                brake_image = ImageTk.PhotoImage(image=img)

                # **แสดงภาพนิ่งใน GUI**
                camera_label.imgtk = brake_image
                camera_label.configure(image=brake_image)

                # **แสดงจำนวนชิ้นงานปัจจุบัน (ขึ้นข้อความทันที)**
                detected_p38 = int(p38_var.get())  # จำนวน P38 ที่ตรวจจับได้
                messagebox.showinfo("🔴 Brake Mode", f"📌 Layer: {current_layer}\n🔍 P38 Detected: {detected_p38}")

    else:
        # **ออกจากโหมด Brake**
        is_brake_mode = False
        update_status("Processing")  # ✅ กลับสู่สถานะปกติ
...
# ✅ เพิ่มปุ่ม Brake ลงใน GUI
brake_button = tk.Button(button_frame, image=images["break"][1], borderwidth=0, command=toggle_brake)
brake_button.grid(row=2, column=0, padx=5, pady=5)
brake_button.bind("<Enter>", lambda e: on_hover(brake_button, images["break"][0]))
brake_button.bind("<Leave>", lambda e: on_leave(brake_button, images["break"][1]))
